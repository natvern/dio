...
{\color{blue}Focus on (1), it makes your contribution stronger. Don't
treat (1), (2) and (3) as challenges in the same level. Actually, I
would even go as far as renaming this section ``Reward Shaping'' or
something like this.}
...
{\color{blue}In this next paragraph, I think it important to mention
two things. First of all, even though there is evidence about the
importance of a more informed reward function, many times people would
still go with the more naive function that only rewards final states,
and hopes the computer will learn the quality of each step as it
explores the states (add citations!! Mention works that use this
reward strategy). Secondly, the former is done probably because
incorporating domain knowlegde when states are abstractly represented
may be difficult.}
...
(\emph{i.e.} the chess example)\giselle{Reference?}. Similarly, one should avoid only 
negative rewards as that results in unwanted behavior\giselle{Be more
specific, and if there is a reference, cite it.}. Furthermore, if dealing with a continuous state space, it helps to have a polynomial differential function as the reward function 
...
%
\st{\dio{} does not rely on an abstract representation to infer a reward function, rather only needs 
...
dilemma.}\giselle{Make this part about RL only, and motivate the need
for \dio{} at the end.}

{\color{blue}The next two paragraphs should be greatly shortned and
given less importance. The value in them is that it emphasizes the
importance of a good reward function and thus the relevance of your
contribution. But if they are made too long, it conveys the idea that
there are still these big issues to solve, in spite of a good reward
function, which decreases the relevance of your contribution. Do you
see this?}
...
{\color{blue}Make this specific to the challenge of reward shaping, as
opposed to the three challenges above.}
...
towards meta-learning?\giselle{This sentence is not directly related to your
contribution and may be confusing to readers. I suggest leaving it
out.}

{\color{blue}I suggest moving the paragraph below to the related work
part, and dedicating this section more to motivating \dio{} and how it
makes reward shaping easy.}
...
techniques?} for reinforcement learning, as to tackle its challenges.

{\color{blue}This is the place to talk about \dio{}, the part which I
crossed out before.}
