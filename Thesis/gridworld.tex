...
\gls{episode}\giselle{You need to explain what an episode is before reaching
this point}, hereby ends. An episode is defined as the collection of steps before we reach a terminal state, either 
...
environment. Precisely, we define the state as follows. \samar{fixed.}
\begin{equation*}
  S_t = [x, y, d, G]
\end{equation*}
...
of the agent and does not represent the entire grid. \giselle{But not the whole grid, correct?} \samar{fixed.}
The action space is, 
\begin{equation*}
  A_t = \{ right: 0, up: 1, down: 2, left: 3 \}
\end{equation*}
Finally, the reward is a function of the distance from the goal
defined as,\giselle{Explain that \emph{max\_steps} is the Manhatan
distance, and the intuition behind this function.} 
\begin{equation*}
...
an obstacle, to encourage staying away from the dynamic obstacles. \samar{fixed}
...
\begin{multicols}{2}[\medskip]
    ...
    frames\giselle{You need to explain what a frame is before this
    point.} for a similar start configuration as shown in Figure~\ref{fig:gridrl}. The episode ends when the agent 
...
\giselle{I think it
makes sense to explain what problog is already in the section where
you explain the DIO architecture. So that when we reach this point, we
know what to expect.}: a probabilistic prolog that allows us to capture 
...
The rules of DIO take the following form: \samar{Should I move the Problog procedure up? I think it wouldn't fit but I'm not sure.}
...
%     \end{figure}
    ...
    dio\giselle{Create a command for DIO so that it is typeset
    consistently.} is beneficial even in a stochastic environment. 
